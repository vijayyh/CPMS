...
import { IndianRupee, FileText, Truck, Building2, ArrowRight } from "lucide-react";
...
export default async function AccountsDashboard() {
  ...
  const [totalSpendAgg, openPOs, pendingGRNs, activeVendors, recentPOs] = await Promise.all([
  ...
  const totalSpend = totalSpendAgg._sum.grandTotal ?? 0;
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--text-secondary)]">Track procurement spend, invoices, and vendor payments.</p>
        </div>
        <Link href="/finance" className="flex items-center gap-2 bg-[var(--bg-card)] px-4 py-2 rounded-lg border border-[var(--bg-border)] shadow-sm text-sm font-semibold text-[var(--brand-primary)] hover:bg-[var(--bg-hover)] transition-colors">
          Full Finance Dashboard <ArrowRight size={14} />
        </Link>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard icon={<IndianRupee size={20} />} color="green" label="Total Spend" value={formatCurrency(totalSpend)} />
        <StatCard icon={<FileText size={20} />} color="blue" label="Open Purchase Orders" value={String(openPOs)} />
        <StatCard icon={<Truck size={20} />} color="orange" label="Pending GRN Confirmations" value={String(pendingGRNs)} />
        <StatCard icon={<Building2 size={20} />} color="purple" label="Active Vendors" value={String(activeVendors)} />
      </div>
      ...
      <div className="glass-panel overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {recentPOs.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-[var(--text-muted)]">No purchase orders yet.</td></tr>
              ) : (
                recentPOs.map((po) => (
                  <tr key={po.id} className="border-b border-[var(--bg-border)] hover:bg-[var(--bg-hover)] transition-colors">
                    ...
                    <td className="p-4 text-[var(--text-secondary)]">{formatDate(po.createdAt)}</td>
                    <td className="p-4 font-semibold text-[var(--text-primary)]">{formatCurrency(po.grandTotal)}</td>
