  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        setProjectName("Error looking up project");
      } finally {
          ...
          <form onSubmit={handleSubmit} className="login-form">
            ...
            {role !== "ADMIN" && (
              <div className="form-group">
                <label className="form-label">Project Join Code (Optional)</label>
                <div style={{ position: "relative" }}>
                  <Building2
                    size={15}
                    style={{
                  ...
                  />
                  {isLookingUp && <Loader2 size={14} className="spin" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--brand-primary)' }} />}
                </div>
                {projectName && (
                  <div style={{ fontSize: 12, marginTop: 6, color: projectName.includes("not found") || projectName.includes("Error") ? "var(--color-danger)" : "var(--color-success)", fontWeight: 500 }}>
                    {projectName === "Project not found" ? "Invalid project code" : (projectName.includes("Error") ? projectName : `Joining: ${projectName}`)}
                  </div>
                )}
              </div>
            )}
