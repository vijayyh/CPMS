...
import { Bell, Search, ChevronDown, Sun, Moon, Plus, Settings, LogOut, FileText, AlertCircle, ShoppingCart, CloudSun } from "lucide-react";
import { useState, useEffect, useRef } from "react";
import { signOut } from "next-auth/react";
import { getInitials } from "@cpms/utils";
...
export default function TopBar({ userName, userRole }: TopBarProps) {
  ...
  const markRead = (id: any) => setNotifications(notifications.map(n => n.id === id ? {...n, read: true} : n));

  useEffect(() => {
    fetch('/api/projects/invites')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const inviteNotifs = data.map(inv => ({
            id: inv.id,
            type: "Project Invite",
            text: `Admin ${inv.project.manager?.name || 'User'} invited you to join ${inv.project.name} (${inv.project.code})`,
            read: false,
            variant: "info",
            isInvite: true,
            projectId: inv.projectId
          }));
          setNotifications([...inviteNotifs, ...INITIAL_NOTIFICATIONS]);
        }
      })
      .catch(console.error);
  }, []);

  const handleInviteAction = async (notif: any, action: "ACTIVE" | "REJECTED") => {
    try {
      const res = await fetch('/api/projects/invites', {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ inviteId: notif.id, status: action })
      });
      if (res.ok) {
    ...
    <div className="topbar-wrapper">
      <header className="topbar glass-panel">
        ...
        <div className="topbar-left">
          <div className="topbar-breadcrumb">
            ...
            <span className="breadcrumb-current">
              {pathname.endsWith('/dashboard') ? 'Overview' : (crumbs[crumbs.length - 1]?.label || 'Detail')}
            </span>
          </div>
        </div>
        ...
        <div className="topbar-right">
          ...
          {/* Weather Widget */}
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-[var(--bg-card-solid)] border border-[var(--bg-border-solid)] rounded-full mr-2">
            <CloudSun size={14} className="text-warning" />
            <span className="text-xs font-bold text-[var(--text-title)]">Mumbai, 28°C</span>
          </div>
            ...
            {isNewOpen && (
              <div className="glass-panel menu-dropdown" style={{ width: '220px', left: 0, right: 'auto' }}>
                <div className="menu-group">
                  {!isEmployee && (
                    <>
                      {(userRole === 'ADMIN' || userRole === 'MANAGER') && (
                        <Link href="/projects" className="menu-item" onClick={() => setIsNewOpen(false)}>
                  ...
                  {userRole !== 'ADMIN' && (
                    <Link href="/projects/join" className="menu-item" onClick={() => setIsNewOpen(false)}>
                      <Plus size={14} /> Join a Project
                    </Link>
                  )}
                </div>
              </div>
              ...
              <div className="glass-panel notif-dropdown">
                ...
                <div className="notif-body">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(n => (
                      <div key={n.id} onClick={() => !n.isInvite && markRead(n.id)} className={`notif-item ${n.read ? 'read' : 'unread'}`}>
                        ...
                        <div className="notif-content">
                          ...
                          {n.isInvite && (
                            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                              <button onClick={(e) => { e.stopPropagation(); handleInviteAction(n, 'ACTIVE') }} style={{ padding: '4px 8px', background: 'var(--brand-primary)', color: 'white', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Join</button>
                              <button onClick={(e) => { e.stopPropagation(); handleInviteAction(n, 'REJECTED') }} style={{ padding: '4px 8px', background: 'transparent', color: 'var(--color-danger)', borderRadius: 4, border: '1px solid var(--color-danger)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Decline</button>
                            </div>
                          )}
                        </div>
                      </div>
